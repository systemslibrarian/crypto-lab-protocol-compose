 ...
 *    out of them. `html, body` is a `radial-gradient` between two theme tokens,
 *    so EVERY element that does not paint its own opaque background is text on a
 *    gradient. On top of that, `.card`, `.hex`, `.repeat`, `.explainer`,
 *    `.oracle-mech`, `.mech-byte`, `.crime-secret`, `.crime-reflect`,
 *    `.tmg-bar`, `.code-block` and the SVG's `.d-box` are all
 *    `color-mix(in srgb, var(--panel) N%, var(--bg) M%)`; every `.badge`,
 *    `.spectrum-tag`, `.takeaway`, `.new-here`, `.crime-run-match` and
 *    `.repeat-match` fill is a `color-mix(…, transparent)` over one of those;
 *    and the hero aside is `color-mix(in oklab, var(--focus) 6%, transparent)`.
 *    axe files all of them under `incomplete`, which means a violations-only
 *    gate measured the contrast of almost nothing this lab renders.
 ...
 *    at `.85`, `button:disabled` at `.5` (which every attack button becomes
 *    while its attack runs), and `.oracle-mech.mech-idle .mech-byte` at `.75` —
 *    the state the padding-oracle diagram ships in and returns to after every
 *    reset, so it is what a reader sees for most of their visit.
 *
 ...
 *    boxes are `.page` (`overflow-x: hidden`), the three `.hex` output regions
 *    (`overflow: auto`), `.code-block` (`overflow-x: auto` around
 *    `white-space: pre`, which genuinely scrolls at 380px), `.tmg-bar`
 *    (`overflow: hidden`) and `.sr-only`. Content scrolled or clipped past those
 ...
 *    branch is LIVE on this page, not defensive: `.cbc-diagram` in Exhibit 2's
 *    explainer draws four `<rect>`s and a `<circle>` and then paints six
 *    `<text>` labels over them, all inside one `<g>`. An ancestor-only walk
 *    would composite `Cᵢ (ciphertext)` onto the `<svg>`'s own transparent
 *    background rather than onto the `.d-box` drawn under it, and report a ratio
 *    nothing on screen has — including for `.d-attacker-text`, which is
 *    `--danger` ink over a `color-mix(in srgb, var(--danger) 16%, var(--panel))`
 *    fill and is the tightest measurement in the figure.
 *
 ...
 * rule. `nontext.ts` measures those separately, and this page's only meaningful
 * one is the `<summary>` disclosure triangle on the two `.explainer` details,
 * which is the UA's own marker rather than a stylesheet's.
 *
 * SECOND, `border-bottom: 1px dotted currentColor` on `.gloss` is a non-text
 * cue — it is the entire affordance marking a term as having a definition, since
 * the gloss carries no other visual distinction. It shares `currentColor` with
 * the prose it sits in, so it inherits that prose's measured ratio exactly; no
 * separate measurement is possible or needed.
 */
 ...
 * uses. It can afford to, because under the reduced motion this gate asserts
 * there is no short-lived styling left to miss: `main.ts` reads the preference
 * at import and takes the branch that renders each attack's final frame
 * directly, so `.mech-sweeping` (the 0x00..0xFF byte sweep) and
 * `.tmg-bar-losing` (the losing half of the timing race) are never applied at
 * all, and `.panel`'s reveal animation is cancelled by the stylesheet.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives 90ms is shorter than a full axe
 * pass, so a whole-page scan would catch it only sometimes, and a
 * sometimes-measured assertion is worse than none. Those two classes are
 * therefore measured by hand instead, and the numbers recorded in the commit.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * that is `.page` (`overflow-x: hidden`), the three `.hex` output regions
     * (`overflow: auto`), `.code-block` (`overflow-x: auto` around
     * `white-space: pre`), and `.tmg-bar` (`overflow: hidden`, which is what
     * keeps each timing bar's fill inside its track). `.sr-only` is handled
     * separately below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. Both branches are LIVE on this page: `.cbc-diagram`
      // paints four <rect>s and a <circle> that ARE the backdrop for its six
      // <text> labels, and five stroke-only <line>s (the arrows between the
      // boxes, each `stroke: currentColor` with `fill` left at its initial
      // black) that are not — and two of those lines cross directly under the
      // `Cᵢ₋₁ (prev block)` label.
    ...
    /**
     * `aria-hidden` is NOT skipped here, and that is a deliberate departure from
     * both axe's `color-contrast` rule and this helper's form elsewhere in the
     * fleet.
     *
     * The usual argument for skipping it — that `aria-hidden` marks decorative
     * content, so measuring it is being stricter than axe for no reader's
     * benefit — does not survive contact with this page. `main.ts` puts
     * `aria-hidden="true"` on FIVE elements that carry the lab's actual output:
     * `#oracle-visual` (the plaintext the padding oracle recovers, byte by
     * byte), `#tmg-visual` (the eight timing bars and their per-byte readouts),
     * `#crime-visual` (the recovered session secret), `#oracle-mech` (the live
     * 16-byte CBC block diagram) and `#tls-dots`. Each is paired with a
     * `role="status"` line that narrates the result in prose, which is why the
     * attribute is defensible for AT — a sighted reader is not offered that
     * trade, and reads the hidden element.
     *
     * So this is precisely the shared blind spot the sweep keeps finding: text
     * that is removed from the accessibility tree AND still painted is invisible
     * to axe and would be invisible here too. Measuring it costs nothing (the
     * inks are `--safe`, `--danger`, `--pending` and `--text`, all of which the
     * exposed prose uses as well) and closes the gap. The only thing lost is
     * that a genuinely decorative `aria-hidden` glyph would now be judged — and
     * on this page every one of those is a `·` or an arrow drawn in an ink the
